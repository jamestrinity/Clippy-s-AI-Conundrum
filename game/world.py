...
from .buildings import Factory, Powerplant, Mine
...
class World:
    ...
    def update(self, camera):
        ...
        if self.hud.selected_tile is not None:
            ...
            if self.can_place_tile(grid_pos):
                ...
                if mouse_action[0] and not collision:
                    ...
                    elif self.hud.selected_tile["name"] == "powerplant":
                        ent = Powerplant(render_pos, self.resource_manager)
                        self.entities.append(ent)
                        self.buildings[grid_pos[0]][grid_pos[1]] = ent
                    elif self.hud.selected_tile["name"] == "mine":
                        ent = Mine(render_pos, self.resource_manager)
                        self.entities.append(ent)
                        self.buildings[grid_pos[0]][grid_pos[1]] = ent
        ...
        paperclip_count = self.resource_manager.resources["paperclip"]           
        if paperclip_count >= 100:
            unrender_chance = 0.00001  
            for x in range(self.grid_length_x):
                for y in range(self.grid_length_y):
                    if self.world[x][y]["tile"] == "tree" or self.world[x][y]["tile"] == "rock":
                        if random.random() < unrender_chance:
                            self.world[x][y]["tile"] = ""  # Unrender the grass tile
        elif paperclip_count >= 200:
            unrender_chance = 0.00002 
            for x in range(self.grid_length_x):
                for y in range(self.grid_length_y):
                    if self.world[x][y]["tile"] == "tree" or self.world[x][y]["tile"] == "rock":
                        if random.random() < unrender_chance:
                            self.world[x][y]["tile"] = ""
        elif paperclip_count >= 300:
            unrender_chance = 0.00003 
            for x in range(self.grid_length_x):
                for y in range(self.grid_length_y):
                    if self.world[x][y]["tile"] == "tree" or self.world[x][y]["tile"] == "rock":
                        if random.random() < unrender_chance:
                            self.world[x][y]["tile"] = ""
        elif paperclip_count >= 400:
            unrender_chance = 0.00004  
            for x in range(self.grid_length_x):
                for y in range(self.grid_length_y):
                    if self.world[x][y]["tile"] == "tree" or self.world[x][y]["tile"] == "rock":
                        if random.random() < unrender_chance:
                            self.world[x][y]["tile"] = ""
        elif paperclip_count >= 500:
            unrender_chance = 0.00005  
            for x in range(self.grid_length_x):
                for y in range(self.grid_length_y):
                    if self.world[x][y]["tile"] == "tree" or self.world[x][y]["tile"] == "rock":
                        if random.random() < unrender_chance:
                            self.world[x][y]["tile"] = ""
        elif paperclip_count >= 600:
            unrender_chance = 0.00006  
            for x in range(self.grid_length_x):
                for y in range(self.grid_length_y):
                    if self.world[x][y]["tile"] == "tree" or self.world[x][y]["tile"] == "rock":
                        if random.random() < unrender_chance:
                            self.world[x][y]["tile"] = ""
        elif paperclip_count >= 700:
            unrender_chance = 0.00007  
            for x in range(self.grid_length_x):
                for y in range(self.grid_length_y):
                    if self.world[x][y]["tile"] == "tree" or self.world[x][y]["tile"] == "rock":
                        if random.random() < unrender_chance:
                            self.world[x][y]["tile"] = ""
        elif paperclip_count >= 800:
            unrender_chance = 0.00008  
            for x in range(self.grid_length_x):
                for y in range(self.grid_length_y):
                    if self.world[x][y]["tile"] == "tree" or self.world[x][y]["tile"] == "rock":
                        if random.random() < unrender_chance:
                            self.world[x][y]["tile"] = ""
        elif paperclip_count >= 900:
            unrender_chance = 0.00009  
            for x in range(self.grid_length_x):
                for y in range(self.grid_length_y):
                    if self.world[x][y]["tile"] == "tree" or self.world[x][y]["tile"] == "rock":
                        if random.random() < unrender_chance:
                            self.world[x][y]["tile"] = ""
        elif paperclip_count >= 1000:
            unrender_chance = 0.0001
            for x in range(self.grid_length_x):
                for y in range(self.grid_length_y):
                    if self.world[x][y]["tile"] == "tree" or self.world[x][y]["tile"] == "rock":
                        if random.random() < unrender_chance:
    ...
    def load_images(self):
        ...
        block = pg.image.load("game/assets/block.png").convert_alpha()
        # read images
        building1 = pg.image.load("game/assets/building01.png").convert_alpha()
        building2 = pg.image.load("game/assets/building02.png").convert_alpha()
        tree = pg.image.load("game/assets/tree.png").convert_alpha()
        rock = pg.image.load("game/assets/rock.png").convert_alpha()
        block_inv = pg.image.load("game/assets/block_inv.png").convert_alpha()
